...
# Ask for Input Values
    art_w = art_h = matt_overlap = matt_side = matt_top = matt_bottom = frame_width = rabbet_w = None
...
# Use a while loop to collect inputs until all are valid
    ...
    while True:
        try:
            matt_overlap = float(input("Matt Overlap (How much the matt will overlap the art on each side -- min. 0.125 inch suggested): "))
            break
        except ValueError:
            print("\nInvalid input for matt overlap...\n")

    while True:
        try:
            matt_side = float(input("Desired Matt Width for Sides: "))
            break
        except ValueError:
            print("\nInvalid input for side matt width...\n")

    while True:
        try:
            matt_top = float(input("Desired Matt Width for Top: "))
            break
        except ValueError:
            print("\nInvalid input for top matt width...\n")

    while True:
        try:
            matt_bottom = float(input("Desired Matt Width for Bottom: "))
            break
        except ValueError:
            print("\nInvalid input for bottom matt width...\n")
    ...
    while True:
        try:
            rabbet_w = float(input("Rabbet Width (how much the frame overlaps the matt -- min. 0.25 inch suggested): "))
...
# calculate frame measurements
    inner_frame_w = art_w - (matt_overlap*2) + (matt_side*2)
    inner_frame_h = art_h - (matt_overlap*2) + matt_top + matt_bottom 
    outer_frame_w = inner_frame_w + (frame_width*2)
    outer_frame_h = inner_frame_h + (frame_width*2)

# calculate matt measurements
    inner_matt_w = art_w - (matt_overlap*2)
    inner_matt_h = art_h - (matt_overlap*2)
    outer_matt_w = inner_matt_w + (matt_side*2) + (rabbet_w*2)
    outer_matt_h = inner_matt_h + matt_top + matt_bottom + (rabbet_w*2)

    print("\n########## Input Measurements ############ \n")
    print(f"Art Measurements: {art_w} W x {art_h} H")
    print("Matt Overlap:", matt_overlap)
    print("Matt Side Showing:", matt_side)
    print("Matt Top Showing:", matt_top)
    print("Matt Bottom showing:", matt_bottom)
    print("Frame Width:", frame_width)
    print("Rabbet Width:", rabbet_w)

    print("\n########## Matt Measurements ############ \n")
    print("Inner Matt:",inner_matt_w,"W x",inner_matt_h,"H")
    print("Outer Matt:",outer_matt_w,"W x",outer_matt_h,"H")
...
#alignment variable
    x_align = (win_w/6) - n*(frame_width + matt_side - matt_overlap)
    y_align = 225 - n*(frame_width + matt_bottom - matt_overlap)
...
#Top Writing
    ...
    t.goto(t.pos() + (0, s))
    t.write(f'Matt Bottom Showing: {matt_bottom}"', move=False, font=('Futura', 14))
    t.goto(t.pos() + (0, s))
    t.write(f'Matt Top Showing: {matt_top}"', move=False, font=('Futura', 14))
    t.goto(t.pos() + (0, s))
    t.write(f'Matt Side Showing: {matt_side}"', move=False, font=('Futura', 14))
    t.goto(t.pos() + (0, s))
    t.write(f'Matt Overlap: {matt_overlap}"', move=False, font=('Futura', 14))
    t.goto(t.pos() + (0, s))
...
#draw matt
    t.pencolor("black")
    t.goto( t.pos() + (-matt_overlap*n, matt_overlap*n) )
    t.pendown()
    t.left(90)
    t.forward(inner_matt_h*n)
    t.left(90)
    t.forward(inner_matt_w*n)
    t.left(90)
    t.forward(inner_matt_h*n)
    t.left(90)
    t.forward(inner_matt_w*n)
    t.penup()
    t.pencolor("gainsboro")
    t.goto( t.pos() + ((matt_side + rabbet_w)*n, -n*(matt_bottom + rabbet_w) ))
    t.pendown()
    t.left(90)
    t.forward(outer_matt_h*n)
    t.left(90)
    t.forward(outer_matt_w*n)
    t.left(90)
    t.forward(outer_matt_h*n)
    t.left(90)
    t.forward(outer_matt_w*n)
...
#draw frame
    ...
    t.color("black")
    t.write('Matt Measurements', move=False, font=('Futura', 16))
    t.goto(t.pos() + (0, -1.5*s))
    t.write(f'Inner Matt: {inner_matt_w}" W x {inner_matt_h}" H', move=False, font=('Futura', 14))
    t.goto(t.pos() + (0, -s))
    t.write(f'Outer Matt: {outer_matt_w}" W x {outer_matt_h}" H', move=False, font=('Futura', 14))
    t.goto(t.pos() + (0, -2*s))
